 ...
 * @Description:
 */
public class UnReadReportVo implements Serializable{

    @ApiModelProperty("大数据报告未读数")
    private Integer bigDataUnReadNumber = 0;//大数据报告未读数
    @ApiModelProperty("信贷记录未读数")
    private Integer creditUnReadNumber = 0;//信贷记录未读数
    @ApiModelProperty("不良记录未读数")
    private Integer badUnReadNumber = 0;//不良记录未读数
    @ApiModelProperty("支付宝评测未读数")
    private Integer aliPayUnReadNumber = 0;//支付宝评测未读数

    public UnReadReportVo(){
        super();
    }
    public UnReadReportVo(UnReadReportBo unReadReportBo) {
        this.bigDataUnReadNumber = unReadReportBo.getBigDataUnReadNumber();
        this.creditUnReadNumber = unReadReportBo.getCreditUnReadNumber();
        this.badUnReadNumber = unReadReportBo.getBadUnReadNumber();
        this.aliPayUnReadNumber = unReadReportBo.getAliPayUnReadNumber();
    }
